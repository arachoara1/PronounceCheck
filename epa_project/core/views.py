...
class EvaluatePronunciationView(APIView):
    """
    S3에 업로드된 사용자 음성과 표준 음성을 기반으로 채점 함수 실행
    """
    def post(self, request):
        try:
            # 클라이언트로부터 사용자 및 표준 음성 키 전달받기
...
class UpdatePronunciationScoreView(APIView):
    """
    S3에서 사용자와 표준 음성을 가져와 채점하고 결과를 PostgreSQL에 저장
    """
    def post(self, request):
        ...
        try:
            ...
            # 채점 결과 검증
            required_keys = ["Pitch Pattern", "Mispronounced Words"]
